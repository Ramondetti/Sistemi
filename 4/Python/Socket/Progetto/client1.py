...
class ChatClient:
    def __init__(self, root):
        self.root = root
        self.root.title("Chat Privata")

        self.nickname = simpledialog.askstring("Nickname", "Inserisci il tuo nome:", parent=root)
        ...
        # Frame contatti a sinistra - larghezza fissa maggiore
        self.contacts_frame = tk.Frame(root, bd=2, relief=tk.SUNKEN, width=250)
        self.contacts_frame.pack(side=tk.LEFT, fill=tk.Y, padx=5, pady=5)
        self.contacts_frame.pack_propagate(False)  # blocca il ridimensionamento automatico

        self.contacts_label = tk.Label(self.contacts_frame, text="Contatti", font=("Arial", 14, "bold"))
        self.contacts_label.pack(pady=(5, 10))

        self.users_listbox = tk.Listbox(self.contacts_frame, width=30, font=("Arial", 12))
        self.users_listbox.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        self.users_listbox.bind('<<ListboxSelect>>', self.change_chat)

        # Frame chat a destra - più largo
        self.chat_frame = tk.Frame(root, bd=2, relief=tk.SUNKEN)
        self.chat_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=5, pady=5)

        self.chat_with_label = tk.Label(self.chat_frame, text="Nessuna chat selezionata", font=("Arial", 12, "italic"))
        self.chat_with_label.pack(pady=(5,0))

        self.chat_area = scrolledtext.ScrolledText(self.chat_frame, state='disabled', font=("Arial", 12), wrap=tk.WORD)
        ...
        self.send_button = tk.Button(entry_frame, text="Invia", command=self.send_message, font=("Arial", 12))
    ...
    def change_chat(self, event):
        ...
        if selection:
            new_user = self.users_listbox.get(selection[0])
            self.current_chat = new_user
            self.chat_with_label.config(text=f"Chat con: {new_user}")
            self.display_chat_history()

    def send_message(self, event=None):
        message = self.entry.get().strip()
        if message and self.current_chat:
            formatted = f"{self.current_chat}::{message}"
            try:
                self.sock.send(formatted.encode())
                self.chat_history.setdefault(self.current_chat, []).append(f"Me: {message}")
                self.display_chat_history()
                self.entry.delete(0, tk.END)
            except Exception as e:
                self.chat_area.config(state='normal')
                self.chat_area.insert(tk.END, f"[ERRORE invio messaggio: {e}]\n")
            ...
            try:
                ...
                if msg.startswith("[USERS]::"):
                    ...
                    if ": " in msg:
                        ...
                        self.chat_history.setdefault(sender, []).append(msg)
    ...
    def update_user_list(self, users):
        ...
        if not users:
            self.chat_with_label.config(text="Nessun altro utente online")
            self.chat_area.config(state='normal')
            self.chat_area.delete(1.0, tk.END)
            self.chat_area.insert(tk.END, "Aspettando altri utenti...\n")
            self.chat_area.config(state='disabled')
            self.current_chat = None
    ...
    def display_chat_history(self):
        self.chat_area.config(state='normal')
        self.chat_area.delete(1.0, tk.END)
        messages = self.chat_history.get(self.current_chat, [])
        if not messages:
            self.chat_area.insert(tk.END, "Nessun messaggio in questa chat.\n")
        else:
            for msg in messages:
...
if __name__ == '__main__':
    root = tk.Tk()
    root.geometry("900x600")  # finestra più grande
    ChatClient(root)
    root.mainloop()
=======
